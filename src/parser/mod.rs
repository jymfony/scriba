...
pub fn parse(code: String, filename: Option<&str>) -> Result<Program, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn exec_inner(input: PathBuf) {
        ...
        exec_tr(
            ...
            |t| create_pass(t.comments.clone()),
            &code,
        );
    }

    fn create_pass(comments: Rc<SingleThreadedComments>) -> Box<dyn Fold> {
        ...
        Box::new(chain!(
            ...
            class_properties(Some(comments), Default::default()),
        ))
    }
...
"#;
        ...
        let _ = parsed.compile().expect("Should compile with no error");
    }
}

use crate::parser::util::{ident, ident_name};
...
use swc_ecma_utils::{
    ...
    is_maybe_branch_directive, prepend_stmt, private_ident, prop_name_to_expr_value, replace_ident,
...
impl Decorator2022_03 {
    fn process_param_decorators(&mut self, m: &mut ClassMember) {
        let is_constructor = matches!(m, ClassMember::Constructor(_));
        let mut n = match m {
            ClassMember::Method(m) => m.function.params.take(),
            ClassMember::PrivateMethod(m) => m.function.params.take(),
            ClassMember::Constructor(c) => c
                .params
                .iter_mut()
                .map(|p| match p {
                    ParamOrTsParamProp::TsParamProp(p) => Param {
                        span: p.span,
                        decorators: p.decorators.take(),
                        pat: match &p.param {
                            TsParamPropParam::Assign(pat) => pat.clone().into(),
                            TsParamPropParam::Ident(ident) => ident.clone().into(),
                        },
                    },
        ...
        for (i, p) in n.iter_mut().enumerate() {
            ...
            let init = private_ident!(format!("_param{}_init", i));
            ...
            init_calls.push(Stmt::If(IfStmt {
                span: DUMMY_SP,
                test: Box::new(Bin(BinExpr {
                    span: DUMMY_SP,
                    op: BinaryOp::NotEqEq,
                    left: init.clone().into(),
                ...
                cons: Box::new(Stmt::Expr(ExprStmt {
                    ...
                    expr: Box::new(Expr::Assign(AssignExpr {
                        span: DUMMY_SP,
                        op: AssignOp::Assign,
                        left: param_ident.clone().into(),
                        right: CallExpr {
                            span: DUMMY_SP,
                            callee: Expr::Member(MemberExpr {
                                span: DUMMY_SP,
                                obj: init.into(),
                                prop: MemberProp::Ident(ident_name("call")),
                            })
                            .as_callee(),
                            args: vec![
                ...
                ArrayLit {
                    ...
                    elems: match m {
                        ClassMember::Constructor(..) => {
                            vec![
                                Some(CLASS.as_arg()),
                                Some(Expr::undefined(DUMMY_SP).as_arg()),
                                Some(0_usize.as_arg()),
                            ]
                        }
            ...
            let arg = Some(
                ArrayLit {
                    ...
                    elems: vec![
                        dec,
                        Some(PARAM.as_arg()),
                        name.map(|n| n.sym.clone().as_arg())
                            .or_else(|| Some(Expr::undefined(DUMMY_SP).as_arg())),
                        Some(i.as_arg()),
    ...
    fn process_decorators_of_class_members(&mut self, members: &mut [ClassMember]) {
        for cm in members {
            ...
            self.process_param_decorators(cm);
        }
    }
}
                                                    ...
                                                    arg: Some(
                                                        CallExpr {
                                                            ...
                                                            callee: getter_var
                                                                ...
                                                                .unwrap()
                                                                .as_callee(),
                                                            args: vec![ThisExpr { span: DUMMY_SP }
                                                                .as_arg()],
                                                            ..Default::default()
                                                        }
                                                        .into(),
                ...
                elems: vec![
                    ...
                    Some(
                        match (n.is_static, n.kind) {
                            (true, MethodKind::Method) => METHOD + STATIC,
                            (false, MethodKind::Method) => METHOD,
                            (true, MethodKind::Setter) => SETTER + STATIC,
                            (false, MethodKind::Setter) => SETTER,
                            (true, MethodKind::Getter) => GETTER + STATIC,
                            (false, MethodKind::Getter) => GETTER,
                        }
                        .as_arg(),
                    ),
                    Some(name.as_arg()),
    ...
    fn visit_mut_expr(&mut self, e: &mut Expr) {
        if let Expr::Class(c) = e {
            if !c.class.decorators.is_empty() {
                ...
                *e = SeqExpr {
                    span: DUMMY_SP,
                    exprs: vec![Box::new(e.take()), new.into()],
                }
                .into();
        ...
        for (index, n) in n.iter_mut().enumerate() {
            ...
            })) = n
            {
                if !c.class.decorators.is_empty() {
                    ...
                    insert_builder.push_back(index, ModuleItem::Stmt(stmt));
                    *n = ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultExpr(ExportDefaultExpr {
                        span: *span,
                        expr: new_class_name.into(),
                    }));
